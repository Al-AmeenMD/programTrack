...
export default function AttendanceMarkingPage({ params }: RouteContext) {
  ...
  const fetchRoster = async () => {
    try {
      ...
      const assignedGroups: string[] = json.meta?.facilitator_assigned_group_ids || [];
      setFacilitatorAssignedGroupIds(assignedGroups);

      // Intelligent Smart Default group filter for facilitators:
      if (user?.role === "facilitator") {
        if (assignedGroups.length === 1) {
          setSelectedGroupFilter(assignedGroups[0]);
        } else if (assignedGroups.length >= 2) {
          setSelectedGroupFilter("my_groups");
        } else {
  ...
  const handleMarkAllPresentSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const requestBody: {
        except: Array<{ enrollment_id: string; status: string } | string>;
        course_id?: string;
      ...
      if (
        ...
        requestBody.course_group_id = selectedGroupFilter;
      }
  ...
  // Active group filter label for "Mark All" prompt
  const activeScopeLabel =
    selectedGroupFilter === "my_groups"
      ? "Your Assigned Groups (Combined)"
      : selectedGroupFilter === "unassigned"
      ? "Unassigned Group"
      : selectedGroupFilter !== "all"
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          ...
          <div className="flex items-center space-x-2">
            <button
              ...
              disabled={!session.is_active || filteredRoster.length === 0}
              title={!session.is_active ? "Session is inactive — attendance is read-only" : undefined}
              className={`px-3 py-1.5 font-medium text-xs rounded-md shadow-xs transition flex items-center space-x-1.5 cursor-pointer ${
                session.is_active && filteredRoster.length > 0
                  ? "bg-teal-700 hover:bg-teal-800 text-white"
                  : "bg-slate-100 text-slate-400 border border-slate-300 cursor-not-allowed"
              }`}
            >
              <CheckCheck className="w-4 h-4" />
              <span>Mark {activeScopeLabel} as Present</span>
            </button>
          </div>
        </div>
      ...
      {/* Group Scoping Pill Bar (When Groups Exist in Program/Course) */}
        ...
        <div className="bg-white p-3 rounded-lg border border-slate-200 shadow-xs space-y-2">
          <div className="flex items-center justify-between text-xs">
            ...
            <span className="text-[11px] text-slate-400">
              Showing {filteredRoster.length} of {roster.length} participants
            </span>
          </div>

          <div className="flex flex-wrap gap-2 pt-1">
            {/* Multi-group combined pill for facilitators assigned to 2+ groups */}
            {facilitatorAssignedGroupIds.length >= 2 && (
              <button
                type="button"
                onClick={() => setSelectedGroupFilter("my_groups")}
                className={`px-3 py-1.5 rounded-full text-xs font-semibold transition flex items-center space-x-1.5 cursor-pointer ${
                  selectedGroupFilter === "my_groups"
              ...
              >
                ...
                <span>My Assigned Groups (Combined)</span>
              </button>
            )}

            {/* Individual Group Pills */}
            {availableGroups.map((g) => {
              const isAssigned = facilitatorAssignedGroupIds.includes(g.id);
              const isSelected = selectedGroupFilter === g.id;
              const count = roster.filter((r) => r.course_group_id === g.id).length;
              ...
              return (
                <button
                  ...
                  className={`px-3 py-1 rounded-full text-xs font-medium transition flex items-center space-x-1.5 cursor-pointer ${
                    isSelected
                ...
                >
                  <span>{g.name}</span>
                  <span className={`text-[10px] px-1.5 py-0.2 rounded-full ${isSelected ? "bg-teal-800 text-white" : "bg-white/80 text-slate-600"}`}>
                    {count}
                  </span>
                  {isAssigned && <span className="text-[10px] text-teal-600 font-bold">★</span>}
                </button>
              );
            })}

            {/* All Track Participants Pill */}
            <button
              type="button"
      ...
      <div className="bg-white rounded-lg border border-slate-200 shadow-xs overflow-hidden">
        {filteredRoster.length === 0 ? (
          <div className="p-8 text-center text-xs text-slate-500">
            No participants match the selected group/course filter or search query.
          </div>
        ) : (
                        ...
                        {session.is_active ? (
                          <div className="inline-flex items-center space-x-1">
                            <button
                              ...
                              className={`px-2.5 py-1 rounded text-[11px] font-semibold transition border cursor-pointer ${
                                rec?.status === "present"
      ...
      <Modal
        ...
        title={`Mark All as Present — ${activeScopeLabel}`}
        maxWidth="xl"
      >
        <form onSubmit={handleMarkAllPresentSubmit} className="space-y-4 text-xs">
          <div className="p-3 bg-teal-50 border border-teal-200 text-teal-800 rounded-md text-[11px]">
            <span className="font-bold">Safe Bulk Operation:</span> This action will mark all currently <span className="font-semibold">unmarked</span> participants within <span className="font-bold underline">{activeScopeLabel}</span> as <span className="font-semibold text-emerald-800">Present</span>. Pre-marked attendance records (including existing Absent/Late/Excused entries) will NOT be overwritten.
          </div>

          <div>
            <h4 className="font-semibold text-slate-700 mb-1.5">Specify Exceptions (Optional):</h4>
            <p className="text-slate-500 text-[11px] mb-2">
              Select any participant below to set an exception or exclude them from being marked present.
            </p>

            <div className="max-h-60 overflow-y-auto border border-slate-200 rounded divide-y divide-slate-100 p-2">
              {filteredRoster
                .filter((item) => !item.attendance_record) // Only unmarked participants in current scope
                .map((item) => {
